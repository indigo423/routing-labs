...
docker exec -it routing-lab-01-pc1 ping -W 0.5 -c 1 198.51.100.1 2>&1 > /dev/null
checkError "${?}"
echo -n " - Test connectivity to PC2 (198.51.100.25/28):  "
docker exec -it routing-lab-01-pc1 ping -W 0.5 -c 1 198.51.100.25 2>&1 > /dev/null
checkError "${?}"
echo -n " - Test connectivity to PC3 (198.51.100.41/28):  "
docker exec -it routing-lab-01-pc1 ping -W 0.5 -c 1 198.51.100.41 2>&1 > /dev/null
checkError "${?}"
echo -n " - Test connectivity to PC4 (198.51.100.57/28):  "
docker exec -it routing-lab-01-pc1 ping -W 0.5 -c 1 198.51.100.57 2>&1 > /dev/null
checkError "${?}"
echo -n " - Test connectivity to PC5 (198.51.100.73/28):  "
docker exec -it routing-lab-01-pc1 ping -W 0.5 -c 1 198.51.100.73 2>&1 > /dev/null
checkError "${?}"
echo -n " - Test connectivity to PC6 (198.51.100.89/28):  "
docker exec -it routing-lab-01-pc1 ping -W 0.5 -c 1 198.51.100.89 2>&1 > /dev/null
checkError "${?}"
